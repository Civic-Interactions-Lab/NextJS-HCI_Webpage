...
- **Feature 15 (Remove Unused Components & Dependencies)** — Deleted all unused/leftover files identified after the redesign: `HighlightStatement.tsx`, `TabBarClient.tsx`, `PersonCard.tsx` (replaced by `person-card.tsx`), `RoleLegend.tsx`, `research-category.tsx`, `research-intro.tsx`, `research-overview-card.tsx`, `HCIOpenHouse.tsx`, `SocialEvents.tsx`, and five unused Sanity imageSettings query helpers (`joinImages`, `pathwaysImages`, `peopleImages`, `researchImages`, `sponsorsImages`). Removed `imageSettings.ts` Sanity schema and its registration from `schemaTypes/index.ts`. Extracted the Convex + Clerk annotation/feedback overlay system to `~/Desktop/hci-annotator/` (with convex backend, all annotation components, providers, and a README for future Chrome extension work), then stripped it from the project — deleted `src/modules/annotations/`, `src/providers/ConvexClientProvider.tsx`, and the `convex/` root directory. Uninstalled `@clerk/nextjs` and `convex` packages via pnpm. Removed `ClerkProvider` wrapper from `src/app/layout.tsx`. Both layouts (`(home)` and `(core)`) cleaned of `AnnotationOverlayWrapper`. Build clean with no errors.
  ...
  - **Feature 15.2 (Quote Board + IDE cleanup)** — Added `quote-board.tsx` (formerly `lab-quotes-cloud.tsx`) to the current members people view, placed between the members grid and the section nav links. Renders an SVG word cloud of full quote sentences from the HCI Lab Memory Wall sticky notes; uses canvas-based text measurement and an Archimedean spiral placement algorithm for tight horizontal-only layout. Cursor proximity interaction: `mousemove` on the SVG converts client coordinates to SVG space via `getScreenCTM`, lerps each quote's scale based on distance (130px radius, max 1.28×) on every `requestAnimationFrame` — smooth without CSS transitions. Brand colors (Well Red `#AA2C45`, Thunder `#2d2d2d`), italic Georgia quotes with name attribution below each. Removed `.idea/` from git tracking (`git rm -r --cached`) and added `.idea/` to `.gitignore` to stop WebStorm IDE files from appearing in `git status`.

---

- **Feature 16 (Codebase Cleanup & Refactor)** — Broad housekeeping pass across the whole app. Merged `Logo.tsx` + `LogoClickHandler.tsx` into a single `logo.tsx`; unified the previously-duplicated navbar/footer nav arrays into one shared `constants/nav.ts`. Navbar breakpoint fixed so the hamburger shows below `lg` instead of `md`; logo and "Temple HCI Lab" text both link home; fixed a nested `<a>` hydration bug in the mobile nav. Removed all Sanity-driven image settings in favor of hardcoded images from `public/`, dropped `Suspense`/skeleton loading from the home view (queries are fast enough not to need it), and deleted a long tail of dead code: unused components, an unused nav-items constant, an unused Sanity query helper, and stale `.unlighthouse` audit output.
...
- **Feedback (Post-Launch Fixes & Polish)** — Round of fixes from user feedback after the redesign shipped.
  ...
  - Replaced the `open-house-2.jpg` placeholder image site-wide with `HCI_OpenHouse-5.jpg`.
  ...
  - Home hero (`home-hero.tsx`): fixed the "Check out our research focus" heading and research cards overlapping the slide-content panel/title on short-viewport laptop screens. Added an `isCompact` state (viewport height < 850px) that shrinks card padding/text/icons and the title slide, mirroring the existing mobile-width breakpoint pattern. The card-position clamp now also reserves space for the heading's own measured height and for the title block (previously only accounted for the slide-content text panels), and the heading's Y position is now clamped against that same floor instead of being computed independently — it could previously swing back above the title once the card clamp engaged on very short viewports.
  - Research cards in the home hero: "See more" links now have a rounded pill border matching the site's other pill-button pattern so they read as clickable.
  - Fixed glitchy hero scroll on mobile: the title slide and slide-content panels used percentage-based `top`/`bottom` positioning, which stretched/squished their box in real time as the hero's height is JS-shrunk on scroll (the collapse effect). Switched mobile to a fixed translate-based anchor (`top-[60%] -translate-y-1/2`) so panels move as a rigid block instead of warping; split the slide-switch horizontal transform onto a separate wrapper `div` since it was colliding with the new Tailwind translate class on the same element (inline `style.transform` silently overrides a Tailwind transform utility on the same node).
