...
/** Fades in the ".nav-card" links belonging to this instance only. */
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from(".nav-card", {
        ...
        duration: 0.7,
        ease: "power2.inOut",
        scrollTrigger: { trigger: ".nav-card", start: "top bottom", once: true },
      });
...
/** List of "quick nav" cards linking to related pages — used across the
 * people, sponsors, and research modules (e.g. Alumni/Collaborators,
 * Become a Sponsor, other research topics). Same look everywhere: title,
 * tagline, and a bordered pill "Learn More" button. */
const NavCardsList = ({ ariaLabel, items, cta, linkDescription }: NavCardsListProps) => {
  const ref = useRef<HTMLElement>(null);

  useNavCardReveal(ref);

  return (
    <nav ref={ref} aria-label={ariaLabel} className="border-t border-thunder/8">
      {items.map((item, i) => (
        <Link
          key={item.label}
          href={item.href}
          aria-label={linkDescription ? linkDescription(item) : `Learn more about ${item.label} — Temple HCI Lab`}
          className={`nav-card group flex flex-col gap-3 py-8 border-b border-thunder/8 transition-colors ${i % 2 === 1 ? "-mx-6 md:-mx-12 px-6 md:px-12 bg-alabaster" : ""}`}
        >
